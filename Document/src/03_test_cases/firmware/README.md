...
* [情報収集 (Information Gathering) (ISTG-FW-INFO)](#information-gathering-istg-fw-info)
  ...
  * [安全でないバイナリコンパイルオプション (Insecure Binary Compilation Options) (ISTG-FW-INFO-004)](#insecure-binary-compilation-options-istg-fw-info-004)
...
ファームウェアに関連するテストケースカテゴリには、以下のものが特定されています。
...
### 安全でないバイナリコンパイルオプション (Insecure Binary Compilation Options) (ISTG-FW-INFO-004) <a name="insecure-binary-compilation-options-istg-fw-info-004"></a>
...
IoT ファームウェア内のコンパイル済みバイナリは、コンパイラやリンカのセキュリティフラグを通じて有効化される標準的なエクスプロイト緩和機能を欠如していることがあります。位置独立実行形式 (Position Independent Executables, PIE)、スタックカナリア、実行不可 (No-Execute, NX)、再配置読み取り専用 (Relocation Read-Only, RELRO)、FORTIFY_SOURCE といったセキュリティ堅牢化機能はメモリ破損脆弱性に対する重要な多層防御を提供します。IoT ルーターのファームウェアに関する調査では、組み込みバイナリの大部分がスタックカナリアや完全な RELRO などの主要な緩和策を欠如しており、デバイスをメモリ破損の悪用に脆弱にすることが明らかになっています。これらの保護の欠如はファームウェアの脆弱性を悪用することの複雑さを低減します。たとえば、攻撃者は PIE/ASLR を欠くバイナリに対してリターン指向プログラミング (Return-Oriented Programming, ROP) 技法を使用でき、NX 保護を欠くバイナリにシェルコードを注入できる可能性があります。さらに、バイナリのセキュリティ態勢に関する詳細を露呈し、より標的を絞った悪用の戦略を可能にします。実際の影響は、数百万もの組み込み機器や IoT デバイスに影響を及ぼした BusyBox のスタックオーバーフローの CVE-2022-48174 のような脆弱性によって実証されており、スタックカナリアや ASLR の欠如が悪用の障壁を著しく低下しました。
