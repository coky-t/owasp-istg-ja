...
これらのセクションに加え、テストケースには **参考情報** セクションを含むことがあり、それに固有の出典 (言及しているツール、関連する脆弱性や弱点の記録、およびその根拠となる [OWASP IoT Security Verification Standard](https://owasp.org/IoT-Security-Verification-Standard-ISVS/) の要件など) を引用しています。

テストケースを追加したり更新する貢献者は、カタログ全体に適用される編集上の規約を定めた [貢献ガイドライン](https://github.com/OWASP/owasp-istg/blob/main/CONTRIBUTING.md) も参照してください。
